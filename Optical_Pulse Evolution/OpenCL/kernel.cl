...
void evolve_state(__global float2 *X,
	   __global float2 *K, 
	   int id, 
	   uint W,
	   float t){  
...
__kernel void RK4Step(__global float2 *P,
    ...
    //computation of k1
    evolve_state(P, A, K, gID_x, W, t);
	for(int i=0; i<W; i++)
	{
		idx = gID_x*W+i;
		Ps[idx] = X[idx] + dt*K[idx]/6;
		Pm[idx] = X[idx] + dt*K[idx]/2;
	}
    
    //computation of k2
    evolve_state(Pm, A, K, gID_x, W, t);
	for(int i=0; i<W; i++)
	{
		idx = gID_x*W+i;
		Ps[idx] = Xs[idx] + dt*K[idx]/3;
		Pm[idx] = X[idx] + dt*K[idx]/2;
	}	
